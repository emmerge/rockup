...
Host.prototype._prepareTasks = function () {
  ...
  // Preparation:
  tasks.execute("Create app src directories", { command: "sudo mkdir -p /opt/"+appName+"/releases; sudo chown `whoami` /opt/"+appName });
  tasks.execute("Install npm dependencies", { command: "sudo npm install -g forever userdown node-gyp" });
  tasks.execute("Add app user", { command: "sudo id -u meteoruser &>/dev/null || sudo useradd meteoruser" });

  // For each service, initialize the corresponding upstart script:
  this.services.each( function(service) {
    tasks.copy("Configure upstart service "+service.name, {
      src: path.resolve(templateDir, 'upstart.conf'),
      dest: "/etc/init/"+service.name+".conf",
      vars: {
        envName: envName,
        appName: appName,
        hostName: hostName,
