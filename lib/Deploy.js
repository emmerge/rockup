...
Deploy.prototype.push = function ( callbackFn ) {
  ...
  builder.buildAndZip( function () {
    ...
    _.each( config.hostNames(), function(hostName) {
      ...
      tasks.execute("Unpacking bundle", {
        command: "cd "+releaseDir+"; tar xvzf bundle.tar.gz > /dev/null"
      });
      tasks.execute("Symlinking current to release", {
        command: "cd /opt/"+appName+"; rm -f current; ln -s releases/"+releaseName+" current"
      });
      ...
      _.each(services, function(service) {
        ...
        tasks.copy("Uploading "+service.name+" environment", {
        ...
        tasks.executeScript("Starting "+service.name+" service", {
      ...
      sshTasks.run( host.sshSession(), function (results) {
        console.log("Deployment to "+hostName+" complete");
      });
