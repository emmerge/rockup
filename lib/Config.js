...
Config.initialize = function (appName, environment) {

  // Target locations:
  var dirPath = path.resolve('deploy');
  var rockBasename = environment+".json";
  var rockFilePath = path.resolve(dirPath, rockBasename);
  var settingsBasename = environment+"-settings.json";
  var settingsFilePath = path.resolve(dirPath, settingsBasename);

  // Example Sources:
  var sourceRockFile = path.resolve(__dirname, '../templates/rockup.json');
  var sourceSettingsFile = path.resolve(__dirname, '../templates/settings.json');

  // Make Directory:
  if ( ! fs.existsSync(dirPath) ) {
    console.log("Creating config directory:", dirPath.yellow);
    fs.mkdirSync( dirPath );
  } else {
    console.log("Config directory already exists:", dirPath.yellow);
  }

  // Check for rockup file and copy if new:
  if ( ! fs.existsSync(rockFilePath) ) {
    console.log("Placing example rockup file:", rockFilePath.yellow);
    copyFile(sourceRockFile, rockFilePath);
  } else {
    console.log("Rockup config file already exists:", rockFilePath.yellow);
  }

  // Check for Meteor settings file and copy if new:
  if ( ! fs.existsSync(settingsFilePath) ) {
    console.log("Placing example Meteor settings file:", settingsFilePath.yellow);
    copyFile(sourceSettingsFile, settingsFilePath);
  } else {
    console.log("Meteor settings file already exists:", settingsFilePath.yellow);
  }

  console.log("Rockup Project Initialized".bold.green);
