 ...
 * @param {String} environment    Name of the target environment
 * @param {String[]} files        Array of file types to generate. Valid
 *                                options include 'rockup', 'meteor'
 **/
Config.initialize = function (appName, environment, files) {
  ...
  if ( fs.existsSync(targetDir) ) {
    console.log("Config directory already exists:".yellow, targetDir.cyan.bold);
  } else {
    console.log("Creating config directory:".yellow, targetDir.cyan.bold);
    fs.mkdirSync( targetDir );
  }

  // Copy desired template files into target directory:
  _.each(extMap, function( ext, type ) {
    if ( _.findWhere(files, type) ) {
      ...
      if ( fs.existsSync(target) ) {
        ...
        console.log("Creating config for", type, "from template".yellow, basename.cyan.bold);
