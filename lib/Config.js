...
Config.list = function () {
  var rockupDir = path.resolve( process.cwd(), "deploy" );
  // var allFiles = fs.readdirSync(rockupDir);
  // var configFiles = _.filter(allFiles, function(f) { return f.match(/rockup\.json$/); });
  // return _.map(configFiles, function(f) { return f.replace(/\.rockup\.json$/, ''); });
  var allFiles = fs.readdirSync(rockupDir);
...
Config.prototype.appPath = function () {
  return path.resolve(process.cwd(), this.app().path);
};
